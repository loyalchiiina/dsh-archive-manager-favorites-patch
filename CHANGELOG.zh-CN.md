...
## 0.1.40-plus.1 - 本 fork（基线上游 0.1.40）
...
- 说明：以上均基于 MichengAI/dsh-archive-manager v0.1.40，原始功能与版权归属原作者。

## 0.1.40 - 2026-09-13

- 支持直接打开归档会话，在 DSH 原生页面查看完整对话并保持归档继续聊天，也可恢复后打开。
- 修复查看归档后设置遮罩未关闭的问题：打开成功后关闭设置，失败时保留设置并显示错误；宿主导航适配不可用时可恢复后打开。
- 归档打开失败提示随界面语言显示，旧宿主正常加载和打开归档时不再产生多余警告。

## 0.1.39 - 2026-09-11

- 修复与 Codex UI 共存时抢占其工作区列表的问题，保留 Codex UI 的列表样式和专属交互，同时提供归档管理入口。
- 首页工作区选择与会话导航改由 DSH 官方插件提供；保留自定义归档侧栏、目录添加、恢复和批量管理，减少宿主升级时导航接口不同步的问题。
- 隔离侧栏目录同步异常，避免影响其他插件注册目录组件。若 Profile 曾手工禁用官方 `ui-workspace`，升级前请移除该覆盖，否则归档侧栏与设置页无法加载。

## 0.1.38 - 2026-09-11

- 支持 DeepSeek Harness `0.1.5-rc.2`，保留此前四个已支持版本的兼容路径。

## 0.1.37 - 2026-09-10

- 修复 DSH `0.1.5-rc.1` 中归档摘要重建后缓存仍不可用的问题；缺失版本或旧代际缓存会从会话原文重建，后续访问复用有效缓存。

## 0.1.36 - 2026-09-10

- 加固发布流程：统一依赖冷却预检，在 npm 精确版本与提交确认后再同步双语 GitHub Release；已发布版本可直接恢复 Release，无需重复安装或发布。
- 等待 npm latest 传播，避免新版本误标为非 Latest；修复等待截止点的负超时参数和错误提示，保留原始中止原因及普通网络错误。
- 补充发布恢复、CLI、双语说明与截止边界回归；插件运行实现及四版本兼容范围保持不变。

## 0.1.35 - 2026-09-10

- 适配 DSH `0.1.5-rc.1`：补齐工作区选择、打开会话与分叉导航，在新建或打开会话时退出全局面板；切换面板或卸载后不再执行迟到的导航及草稿交接。
- 侧栏打开与分叉接入新版导航服务，保留旧宿主回退；DSH peer 范围精确限定为 `0.1.0-rc.8 || 0.1.1-rc.2 || 0.1.2-rc.1 || 0.1.5-rc.1`，开发依赖固定至 `0.1.5-rc.1`。
- 新增导航与侧栏接线回归，隔离矩阵验证上述四个精确版本；显式补齐官方 Store 在 Node 测试中需要的 Zustand/Immer 依赖。
- 在隔离 DSH `0.1.5-rc.1` Web Profile 完成真实浏览器验收：归档恢复、删除与子会话级联、跨筛选批量删除、工作区与全局面板导航、分叉、正文搜索及重启持久性通过；未调用外部模型。
